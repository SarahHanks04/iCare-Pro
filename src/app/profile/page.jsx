...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    if (token && !user.email) {
      const mockedUsers = JSON.parse(localStorage.getItem("mockedUsers")) || [];
      const storedUser = mockedUsers.find((u) => u.name && u.email) || mockedUsers[0];
      if (storedUser) {
        dispatch(
          loginSuccess({
        ...
        setFormData({
          ...
          email: storedUser.email || "",
          phoneNumber: localStorage.getItem(`phone-${storedUser.email}`) || "",
        });
        setAvatar(
          localStorage.getItem(`avatar-${storedUser.email}`) ||
  ...
  }, [dispatch, user.email, router]);

  // Update formData when user changes (e.g., after state restoration)
  useEffect(() => {
    setFormData({
      name: user.name || "",
      email: user.email || "",
    ...
    setAvatar(
      localStorage.getItem(`avatar-${user.email}`) || "/images/default-avatar.jpg"
    );
  }, [user]);
